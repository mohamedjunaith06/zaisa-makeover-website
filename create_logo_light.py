...
# 1. Create static/logo-light.svg (for dark backgrounds like footer)
logo_light_svg = """<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 700 480" width="100%" height="100%">
  <defs>
    <linearGradient id="lightGold" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" stop-color="#f3d7a5" />
      <stop offset="50%" stop-color="#dfb676" />
      <stop offset="100%" stop-color="#c69a54" />
    </linearGradient>
  </defs>

  <g transform="translate(35, 10)">
    <!-- 1. CROWN (Above Z) -->
    <g transform="translate(86, 112) scale(0.68)" fill="url(#lightGold)">
    ...
    <!-- 2. WOMAN HEAD PROFILE WITH BUN (Above I, center at x = 315) -->
    <g transform="translate(268, 25) scale(0.95)" fill="url(#lightGold)">
      <!-- Bun on top left -->
      <path d="M 32 35 C 22 24, 10 26, 4 37 C -2 48, 1 60, 11 67 C 22 74, 33 65, 34 54 C 34 46, 33 40, 32 35 Z" />
      <path d="M 12 36 C 18 28, 28 31, 33 39 C 35 44, 34 51, 29 56 C 23 60, 14 56, 11 49 C 9 44, 10 39, 12 36 Z" fill="#34041f" opacity="0.35" />
    ...
    <!-- 3. 'ZAISA' (Pure Cream/White for maximum elegance on dark bg) -->
    <g fill="#fbf3ea">
      <!-- Z -->
    ...
    <!-- 4. 'MAKEOVER' SUBTITLE (Champagne Gold) -->
    <text x="315" y="348" 
          font-family="'Manrope', 'Montserrat', 'Century Gothic', sans-serif" 
          font-size="32" 
          font-weight="600" 
          letter-spacing="10" 
          fill="#e6cd9b" 
...
with open('static/logo-light.svg', 'w', encoding='utf-8') as f:
    f.write(logo_light_svg)

# Also create a raster light PNG
# Load transparent logo.png, map dark wine text to cream/white (#fbf3ea) and gold to soft gold (#e6cd9b)
...
for y in range(h):
    for x in range(w):
        r, g, b, a = pixels[x, y]
        if a > 0:
            # If wine color (r high, g low, b low)
            if r > g + 20 and g < 100:
                # Map to cream: #fbf3ea
                out_pix[x, y] = (251, 243, 234, a)
            elif r > b + 20:
                # Map gold: #f0d5a3
                out_pix[x, y] = (240, 213, 163, a)
            else:
                out_pix[x, y] = (251, 243, 234, a)

out_img.save('static/logo-light.png')
print("Successfully generated static/logo-light.svg and static/logo-light.png")
